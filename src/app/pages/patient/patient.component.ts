...
export class PatientComponent implements OnInit {
  ...
  savePatient(){
    ...
    if(this.checkNumber(this.tel)){
      ...
      this._servicePatient.createPatient({telephone:this.tel,adresse:this.adresse,prenom:this.prenom,nom:this.nom,depends_on:JSON.parse(localStorage.getItem("user")).id,access_level:2}).then(res=>{
        console.log(res);
        if(res['status'] == 1){
